...
def detect_anomalies(image_path,
    ...
    def severity_from_bbox(x,y,w,h, rmask):
        ...
        area_norm=min(1.0,(w*h)/(H*W*0.05))
        ...
        while changed:
            ...
            for j in range(len(items)):
                ...
                if iou((x,y,w,h),items[j]["bbox"])>=iou_thr or gap_px((x,y,w,h),items[j]["bbox"])<=GAP:
                    ...
                    w=max(x+w,x2+w2)-x
                    h=max(y+h,y2+h2)-y
                    umask=cv2.bitwise_or(umask, items[j]["mask"])
                    cnts.append(items[j]["cnt"])
                    changed=True
...
# === keep only the top 4 anomalies ===
    ...
    def shape_features(a):
        ...
        ecc=pca_eccentricity(cnt)
        sk = skeletonize_binary(a["mask"])
    ...
    def classify_fault(a):
        ...
        wire_votes  = 0
        joint_votes = 0
        if ar_rect >= wire_ar_min:      
            wire_votes += 1
        if ecc >= ecc_thr:              
            wire_votes += 1
        if circ <= circularity_thr:     
            wire_votes += 1
        if extent <= wire_extent_max:   
            wire_votes += 1
        if sk_norm >= wire_sk_norm_min: 
            wire_votes += 1
        if thin:                        
            wire_votes += 1
        if long:                        
        ...
        if extent >= joint_extent_min:        
            joint_votes += 1
        if (thick_est/float(max(1,length))) >= joint_thick_len_min: 
            joint_votes += 1
        if sk_norm <= joint_sk_norm_max:      
            joint_votes += 1
        if (a["area_px"]/(H*W)) <= joint_area_frac_max: 
            joint_votes += 1
        if circ >= joint_circularity_min:     
            joint_votes += 1
    ...
    for a in merged:
        ...
        if hotspot is not None: 
            m["hotspot_xy"]=(int(hotspot[0]), int(hotspot[1]))
        out_meta.append(m)
...
def interface(image_path):
    ...
    annotated_img, heatmap_img, meta = detect_anomalies(image, **thresholds)
